...
| **追踪更新** | 完成编码后更新 [`页面清单.md`](原型图/页面清单.md) 实现状态，并 **同步修订本文**（见 §1.4） |
...
| **S8 测试与论文** | 🔄 进行中 | **Bug 修复 + §十七已批准修订**（需求模块 UI/流程优化，非新业务节点） |

详见 [`分工/项目总体规划.md`](项目总体规划.md) §六、§七、**§十七**。
...
| 序号 | 文档 | 更新内容 |
|:--:|------|----------|
| 1 | **本文**（`代码编写文档.md`） | API 参数、组件约定、状态机、实现锚点（§十六） |
| 2 | [`项目总体规划.md`](项目总体规划.md) | §17.11 实现状态对照表 |
| 3 | [`页面清单.md`](原型图/页面清单.md) | 对应 page_id「实现状态」 |
| 4 | [`系统测试文档.md`](系统测试文档.md) | 新增/变更用例（若有） |

> **禁止** 只改代码不更新文档；规划层变更须先改总体规划 §十七，再改本文，最后写代码（见总体规划 §3.4）。
...
| 后端 | Spring Boot + MyBatis-Plus + Spring Security JWT | Boot 3.2；**MyBatis-Plus 3.5.7**（3.5.9+ 分页须额外引入 `mybatis-plus-jsqlparser`） |
...
src/main/java/com/gzpprod/center/
  common/                    # 统一响应、异常、JWT、状态枚举、SecurityUtils、PageResult、FileMediaTypes
  config/                    # MybatisPlusConfig（分页插件）、UploadStorage、SecurityConfig
  module/
    auth/                    # 登录、当前用户
    demand/                  # 中试需求管理
    evaluation/              # 中试评估管理
    ...
    archive/                 # 中试档案管理
    notification/            # 消息、待办
    common/                  # 跨模块：FileController、ProjectController、ProjectProgressService
  每个 module 内：
...
src/
  ...
  api/http.ts                # axios 实例、拦截器、统一错误处理（含 body.code=401）
  api/file.ts                # fetchFileBlob / downloadBlob（JWT 附件）
  components/                # StatusTag、ProjectStepBar、MaterialUpload、DemandPageHeader 等
...
| 8 | 文档 | 页面清单 + **本文 §十六** + 总体规划 §17.11 | 追踪同步 |
...
2. **公共组件**（需求模块 §17.8—§17.10 必用）：
   - 企业需求子页：右上角 **返回项目列表** → `DemandPageHeader.vue`
   - 中心调度/审核办理态：右上角 **返回待办列表** → `CenterPageHeader.vue`（有 `projectId` 时隐藏待办表）
   - 需求办理中：**五阶段** → `DemandPhaseStepBar.vue`（`phaseSteps`）
   - 跨模块进度详情：**单条**模块步骤条 → `DemandPhaseStepBar` + `moduleSteps`（`GET /api/projects/{id}/progress`）
   - 状态展示 → `StatusTag`
   - 附件 → `MaterialUpload`（上传 + **弹窗预览/下载**，禁止裸链 `window.open`）
   - 列表分页 → `ListPagination.vue`（`page` / `pageSize` / `total`）
3. **详情+办理合一**（§17.9 R9 / §17.10 R14）：
   - 企业进度页 `ProgressView.vue`：`ACCEPTED` 签收、`RETURNED` 补充，与进度条同页
   - 中心审核页 `VerifyView.vue`：详情 + 三种决定按钮同页
   - 调度员工作台 `WorkbenchView.vue`：`RECEIPTED` 同页「确认归档」
   - `ReceiptView` / `SupplementView` / `ArchiveView`（需求归档）→ **重定向**至进度页或工作台
4. **侧栏菜单**：五模块名称字面一致——中试需求管理、中试评估管理、中试调度管理、中试反馈管理、中试档案管理
5. **路由守卫**：非本角色路由重定向首页或 403
6. **样式**：主色 `#1890FF`；成功 `#67C23A`；警告 `#E6A23C`；危险 `#F56C6C`

### 4.4 小程序页面编写要点

1. 请求走 `api/request.js`，token 存 `uni.setStorageSync('token')`
2. 首页待办点击走 `utils/nav.js` 映射到业务页（企业 `RETURNED`/`ACCEPTED` → `progress`）
3. 企业项目列表：阶段 **下拉** + `keyword` 搜索；变更筛选/搜索重置第 1 页
4. 进度页 `progress.vue`：跨模块 `moduleSteps` + 签收/补充操作区（与 Web 同规则）
5. 字段与 Web 同源 API，UI 可简化但**业务字段不可少**
6. 从业务页返回首页时刷新 dashboard（`onShow` 拉取）

### 4.5 页面类型与必用能力

| 页面类型 | Web 组件/模式 | 典型 API |
|----------|---------------|----------|
| 列表+待办 | TodoTable / 首页卡片 + 「办理」+ `ListPagination` | `GET /api/demand/todos?page=&pageSize=` |
| 企业项目列表 | `DemandPageHeader` + 阶段 **下拉** + 搜索框 + `ListPagination` | `GET /api/demand/enterprise/projects?stageFilter=&keyword=` |
| 表单填报 | el-form + 暂存/提交 | `POST .../submit`、`.../draft` |
| 审核确认（合一） | 只读详情 + 意见 + 通过/退回/不予受理 | `POST .../verify`（一步完成） |
| 进度详情（合一） | 单条 `moduleSteps` + 时间线 + 动态操作区 | `GET /api/projects/{id}/progress` |
| 签收/补充 | 嵌入进度页操作区，非独立路由 | `POST .../receipt`、`.../supplement` |
| 统计图表 | ECharts | `GET /api/archive/cycle-stats` 等 |
| 归档 | 工作台同页确认或清单页 | `POST .../archive` |

**附件预览规范（§17.9 R8 / §17.10 R10）：**

- 预览：`MaterialUpload` 内 `el-dialog`，JWT 拉 Blob → Object URL
- 分流：图片 `<img>`、PDF iframe、docx（`docx-preview`）、xlsx（`xlsx` 转 HTML）；扩展名兜底（服务端 `Content-Type` 由 `FileMediaTypes` 按文件名解析）
- 下载：独立按钮，走 `api/file.ts` → `GET /api/files/download?filename=`
- **禁止**：裸链打开 `/uploads/**`、预览时自动触发浏览器下载
...
| `ACCEPTING` | 已登记，待审核 | 需求受理登记 → 受理材料审核（**核验与受理决定一步完成**） |
| `RETURNED` | 已退回待补充 | 需求退回通知 → 材料补充 ↺ |
| `ACCEPTED` | 同意受理 | 受理结果通知 |
| `RECEIPTED` | 已签收 | 受理回执签收 |
| `ARCHIVED` | 需求段归档 | 受理信息归档 |

> **说明：** 原 `VERIFYING` 在实现层已合并进 `ACCEPTING`；审核员 `POST .../verify` 在同一接口完成「材料不齐退回」与「同意/不同意受理」。
...
stateDiagram-v2
  ...
  SUBMITTED --> RETURNED: 调度员退回（仅 SUBMITTED）
  ACCEPTING --> RETURNED: 审核员材料不齐
  RETURNED --> SUBMITTED: 企业补充材料
  ACCEPTING --> ACCEPTED: 审核员同意受理
  ACCEPTING --> Closed: 审核员不同意受理
  ACCEPTED --> RECEIPTED: 企业回执签收
  RECEIPTED --> ARCHIVED: 调度员确认归档
  ARCHIVED --> [*]: stage→EVALUATION
```
...
| 操作 | 允许角色 | 允许状态 |
|------|----------|----------|
| `POST .../demand/reject` | 调度员 | **`SUBMITTED` only** |
| `verify(complete=false)` | 审核员 | `ACCEPTING` |
| 任意退回 | — | **`ACCEPTED` 及之后禁止** |
...
| 五模块步骤条 | 嵌入详情页 | `GET /api/projects/{id}/progress` | 跨模块动态 `moduleSteps`（§17.9 R6） |
| 附件上传 | 表单内 | `POST /api/files/upload` | 各表单页 |
| 附件下载/预览 | MaterialUpload | `GET /api/files/download?filename=` | **须 JWT**；`Content-Type` 见 `FileMediaTypes` |
...
| 进度详情 | `DemandPhaseStepBar`（单条 moduleSteps）+ 时间线 | 跨模块进度；禁止双步骤条 |
| 企业项目列表 | `DemandPageHeader` + 阶段下拉 + 搜索 + `ListPagination` | §17.14 v1.10 |
...
- 分页：查询参数 `page`（默认 1）、`pageSize`（默认 10，最大 100）；响应 `PageResult`：`{ records, total, page, pageSize }`
...
| GET | `/api/demand/todos` | 调度/审核/企业 | 需求待办（**分页** `PageResult`） |
| GET | `/api/demand/enterprise/projects` | 企业 | 我的需求项目（**分页** + **`stageFilter`**，见 §7.3.1） |
| GET | `/api/notifications` | 已登录 | 消息列表 |
| PUT | `/api/notifications/{id}/read` | 已登录 | 标记已读 |
| POST | `/api/files/upload` | 已登录 | 附件上传 |
| GET | `/api/files/download` | 已登录 | 附件下载/预览（`filename` 参数，inline） |
| GET | `/api/projects/{id}` | 相关角色 | 项目详情 |
| GET | `/api/projects/{id}/progress` | 相关角色 | **跨模块**进度：`moduleName` + `moduleSteps` + logs |

### 7.3 需求模块

#### 7.3.1 企业项目列表 `GET /api/demand/enterprise/projects`

| 参数 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `page` | int | 1 | 页码 |
| `pageSize` | int | 10 | 每页条数（最大 100） |
| `stageFilter` | string | `ALL` | `ALL` \| `DEMAND` \| `EVALUATION` \| `DISPATCH` \| `FEEDBACK` \| `ARCHIVE` \| `CLOSED` |
| `keyword` | string | — | 可选；`project_no` / `title` **模糊 OR** 匹配（大小写不敏感）；1—50 字符 |

- 仅返回企业本人项目；排除需求 **草稿**
- `stageFilter` 非法 → 400；`keyword` 超长 → 400
- **`stageFilter` 与 `keyword` AND 组合**；变更任一条件重置 `page=1`
- Web：`ProjectListView.vue` — `el-select` 阶段下拉 + `el-input` 搜索/重置
- 小程序：`projects.vue` — `picker` 阶段选择 + 搜索框

#### 7.3.2 项目级接口 `/api/projects/{id}/demand/*`

| 方法 | 路径 | 页面 / 锚点 | 状态变迁 / 备注 |
|------|------|-------------|-----------------|
| POST | `.../draft` | 需求预览确认页 | → DRAFT |
| POST | `.../submit` | 需求填报页 | DRAFT→SUBMITTED；**11 位手机号**校验 |
| POST | `.../accept-register` | 需求受理工作台 | SUBMITTED→ACCEPTING |
| POST | `.../verify` | 受理材料审核页 `VerifyView` | `complete=false`→RETURNED；`complete=true`+`accepted`→ACCEPTED/CLOSED |
| POST | `.../reject` | 退回意见页 / 工作台 | **仅 SUBMITTED** → RETURNED（调度员） |
| POST | `.../supplement` | 进度页操作区 `ProgressView` | RETURNED→SUBMITTED；**`replaceMaterials`** 整版替换 |
| POST | `.../receipt` | 进度页操作区 | ACCEPTED→RECEIPTED |
| POST | `.../archive` | 工作台同页「确认归档」 | RECEIPTED→ARCHIVED, stage→EVALUATION |
| GET | `.../preview` | 各详情页 | 材料默认 **最新 version** + `fileUrl` 去重 |
| GET | `.../progress` | — | 需求段只读详情；**跨模块进度请用** `GET /api/projects/{id}/progress` |

**待办 route（企业）：** `RETURNED` / `ACCEPTED` → `/enterprise/demand/progress`（§17.10 R14）

**待办 route（调度员）：** 需求段全部（含 `RECEIPTED`）→ `/center/dispatch/demand/workbench`（§17.12 R15）

#### 7.3.3 材料补充去重（§17.10 R13）

| 层 | 实现 |
|----|------|
| 后端 `supplement` | `replaceMaterials(demandId, items, nextVersion)`，禁止无差别 `saveMaterials` 追加 |
| 后端 `getDetail` | `listLatestMaterials()`：仅 max version，按 `fileUrl` 去重 |
| 前端补充 | 已有材料只读展示；提交时传 **保留项 + 新上传项** 作为当前有效清单 |
...
| 2 | `DemandService` 状态机（含 RETURNED ↺ SUBMITTED、退回规则 §17.10 R12） | 单元测试 ≥2 |
| 3 | `DemandController` + 分页 + `stageFilter` + 权限 | Knife4j 文档 |
| 4 | Web 调度员工作台 + 审核员审核页 + 企业项目列表/进度页 | views/center, enterprise |
| 5 | uni-app 企业需求页（列表筛选、进度合一） | pages/enterprise/demand/* |
| 6 | 联调：材料不齐→退回→补充↺；同意受理→签收→工作台归档 | 用例记录 |
| 7 | §17 专项：预览、办理态 UI、材料去重、跨模块进度 | 见 §十六 |
...
| `ProjectStepBar` | 五模块宏观步骤（门户首页等） |
| `DemandPhaseStepBar` | 需求五阶段或当前模块 `moduleSteps`（**单条**，§17.9 R7） |
| `DemandPageHeader` | 企业页标题 + 右上角返回项目列表 |
| `CenterPageHeader` | 中心办理态标题 + 返回待办列表 |
| `MaterialUpload` | 上传 + JWT 弹窗预览/下载（`docx-preview`、`xlsx`） |
| `ListPagination` | 表格底部分页（`total` / `page` / `pageSize`） |
...
| 预览误报「不支持」 | 下载接口 `Content-Type=octet-stream` | 后端 `FileMediaTypes`；前端 `MaterialUpload` 扩展名兜底 |
| `PaginationInnerInterceptor` 找不到 | MyBatis-Plus 3.5.9+ 拆分 jsqlparser | 用 **3.5.7** 或加 `mybatis-plus-jsqlparser` 依赖 |
| 退回报「当前状态不可退回」 | `reject` 仅在 `SUBMITTED` | `ACCEPTING` 由审核员 `verify(complete=false)` |
...
> 与 [`项目总体规划.md`](项目总体规划.md) §十七 一一对应；编码/走查时以此为准。

### 16.1 后端关键类

| 类 / 方法 | 路径 | 说明 |
|-----------|------|------|
| `DemandService.reject` | `module/demand/service/` | 仅 `SUBMITTED`（R12） |
| `DemandService.verify` | 同上 | 一步核验+受理（R4） |
| `DemandService.supplement` | 同上 | `replaceMaterials`（R13） |
| `DemandService.listLatestMaterials` | 同上 | 最新 version + 去重（R13） |
| `DemandService.listEnterpriseProjectsPage` | 同上 | 分页 + `stageFilter` + `keyword`（§17.14） |
| `ProjectProgressService.getProgress` | `module/common/service/` | 跨模块 `moduleSteps`（R6） |
| `FileController.download` | `module/common/controller/` | JWT + `FileMediaTypes`（R10） |
| `PageResult` | `common/` | 统一分页响应（§17.13） |
| `MybatisPlusConfig` | `config/` | `PaginationInnerInterceptor` |

### 16.2 Web 关键页面 / 组件

| 锚点 | 路径 | 对应修订 |
|------|------|----------|
| `ProjectListView.vue` | `views/enterprise/demand/` | 阶段下拉 + 编号/名称搜索（§17.14 v1.10） |
| `ProgressView.vue` | 同上 | 跨模块进度 + 签收/补充合一（R6/R14） |
| `WorkbenchView.vue` | `views/center/dispatch/demand/` | 办理态 + 同页归档（R11/R16） |
| `VerifyView.vue` | `views/center/audit/demand/` | 办理态 + 一步审核（R4/R11） |
| `MaterialUpload.vue` | `components/` | 弹窗预览（R8/R10） |
| `DemandPageHeader.vue` | `components/` | 企业返回列表（§17.8） |
| `CenterPageHeader.vue` | `components/` | 中心返回待办（R11） |
| `ListPagination.vue` | `components/` | 列表分页（§17.13） |
| `api/file.ts` | `api/` | JWT Blob 预览/下载 |
| `api/project.ts` | `api/` | `fetchProjectProgress` |

### 16.3 小程序关键页面

| 页面 | 路径 | 说明 |
|------|------|------|
| `projects.vue` | `pages/enterprise/demand/` | picker 阶段 + 搜索 + 加载更多 |
| `progress.vue` | 同上 | 跨模块进度 + 签收/补充 |
| `receipt.vue` / `supplement.vue` | 同上 | 重定向至 `progress` |

### 16.4 实现状态

完整 ✅/🟡/❌ 对照见总体规划 **§17.11**；本文随编码更新 API 与锚点，状态位以总体规划为准。
...
| 2026-06-26 | v2.2 | §17.14 v1.10：阶段下拉 + `keyword` 搜索（后端/Web/小程序） |
| 2026-06-26 | v2.1 | §十七落地：分页/`stageFilter`、预览、退回状态机、进度合一、§16 实现锚点；强制编码后同步本文（§1.4） |
